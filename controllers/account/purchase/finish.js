...
module.exports = async function(req, res) {
  ...
  try {
    ...
    await db.query(`
      UPDATE users SET
        subscription = ?, library_size_limit = ?,
        referral = ?
      WHERE user_id = ?
    `, [
      payment.body.info.subscription, payment.body.info.gb,
    ...
    if (payment.body.info.refUserSubscription) {
      await db.query(
        ...
        [payment.body.info.refUserSubscription, payment.body.info.referral.user]
      );
    }
