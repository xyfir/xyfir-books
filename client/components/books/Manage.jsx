import { DatePicker, TextField, Button, Paper } from 'react-md';
...
export default class ManageBook extends React.Component {
  ...
  onDownloadMetadata() {
    ...
    this._identifiers.value.split(', ').forEach(id => {
    ...
    if (!query.isbn) {
      (query.author = this._authors.value), (query.title = this._title.value);
    }
        ...
        res.body.metadata.split('\n').forEach((kv, i) => {
          ...
          switch (key) {
            ...
            case 'Comments':
              ...
              else this._comments.innerHTML = comments;
          }
        });
  ...
  onEmbedMetadata() {
    ...
    request
      ...
      .end((err, res) => {
        ...
        else return App._alert('Metadata embedded');
      });
  }
  ...
  onDeleteFormat(f) {
    ...
    request
      ...
      .end((err, res) => {
        if (err || res.body.error) App._alert('Could not delete format');
        else App.store.dispatch(deleteFormat(this.state.id, f));
      });
  }
  ...
  onUploadCover(file) {
    ...
    request
      ...
      .end((err, res) => {
        ...
        localforage
          .setItem(`cover-${this.state.id}`, file)
          .then(img => {
            // Load new cover
  ...
  onSaveChanges() {
    ...
    if (this._comments.getField) data.comments = this._comments.value;
    else data.comments = this._comments.innerHTML;
  ...
  render() {
    ...
    return (
      <div className="manage-book">
        <Paper zDepth={1} component="section" className="main section flex">
          <TextField
            id="text--title"
            ref={i => (this._title = i)}
            type="text"
          ...
          <div>
            ...
            <Button
              ...
              tooltipPosition="top"
            >
              {this.state.embeddingMetadata ? 'Embedding...' : 'Embed Metadata'}
            </Button>
          </div>
        </Paper>

        <Paper zDepth={1} component="section" className="cover section flex">
          <Dropzone
            ref={i => (this._dz = i)}
        ...
        <Paper zDepth={1} component="section" className="other section flex">
          <TextField
            floating
            id="number--rating"
            min={0}
          ...
          <TextField
            ...
            className="md-cell"
            defaultValue={Object.entries(book.identifiers)
              .map(id => `${id[0]}:${id[1]}`)
              .join(', ')}
          />
        ...
        <Paper zDepth={1} component="section" className="comments section flex">
        ...
        <Paper zDepth={1} component="section" className="formats section flex">
